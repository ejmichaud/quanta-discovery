...
-------------------------------
Uses sparse random projections!
-------------------------------
...
if __name__ == '__main__':
    ...
    parser.add_argument("--proj_dim", type=int, default=5000,
                        help="number of dimensions to project to")
    parser.add_argument("--sparsity", type=float, default=0.999,
                        help="sparsity of random projection matrix")
    ...
    for iblock in tqdm(blocks, desc="outer loop", disable=not verbose):
        Gi = torch.zeros((len(iblock), proj_dim), device=device)
        ...
        for jblock in tqdm(blocks[j_index:], leave=False, desc="inner loop", disable=not verbose):
            Gj = torch.zeros((len(jblock), proj_dim), device=device)
    ...
    # ----- save the results -----
    resultname = f"{model_name}_{step}_{loss_threshold}_{skip}_{num_tokens}_sprand_v1.pt"
